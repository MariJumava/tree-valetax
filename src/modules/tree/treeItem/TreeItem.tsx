...
export const TreeItem = ({
  ...
  return (
    <div onClick={() => onSelect(node)} className={styles.itemContainer}>
      {hasChildren && (
        <IoMdArrowDropright
      ...
      {isSelected && (
        <div className={styles.actionsButton}>
          <IoIosAddCircle onClick={handleAddNodeButtonClick} className={classNames(styles.icon, styles.addIcon)} size={24} />
          {ROOT_TREE_NAME !== node.name && (
            <>
              <MdDriveFileRenameOutline
                ...
                size={24}
              />
              <BiSolidTrash onClick={handleDeleteNodeButtonClick} className={classNames(styles.icon, styles.deleteIcon)} size={20} />{' '}
            </>
          )}
        </div>
      )}
